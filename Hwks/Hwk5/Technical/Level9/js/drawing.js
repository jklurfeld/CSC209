...
function drawTrace(traceArr){
    ...
    for (let i = 0; i < traceArr.length; i++){
        // let point = POINTS[i];
        ctx.beginPath();
        ctx.arc(traceArr[i][0], traceArr[i][1], 20, 0, 2 * Math.PI);
        ctx.lineWidth = 4;
...
// or you could make a file called global.js where you define your global variables so your html isn't so lon

function start(){
    clearInterval(INTERVAL);
    currentSteps = 0;
    let traceArr = [];
    INTERVAL = setInterval(move, 50, traceArr);
}

function move(traceArr){
